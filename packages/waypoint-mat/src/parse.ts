...
function processRow(
  ...
  const freeColumnCount =
    5 - rowspanState.filter((slot) => slot.remaining > 0).length;
...
function parseTableRow($: CheerioAPI, row: NodeLike): CellParseResult[] {
  ...
  $(row)
    ...
    .each((_, cell) => {
      ...
      const parsedRowspan =
        rawRowspan === undefined ? Number.NaN : Number.parseInt(rawRowspan, 10);

      cells.push({
        value: extractCellText($, cell),
        rowspan:
          Number.isNaN(parsedRowspan) || parsedRowspan < 1 ? 1 : parsedRowspan,
      });
    });
...
function parseTable<T extends AnyNode>(
  ...
  const rowspanState: RowspanSlot[] = Array.from({ length: 5 }, () => ({
    value: '',
    remaining: 0,
  }));
