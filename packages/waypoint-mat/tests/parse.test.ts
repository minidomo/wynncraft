...
function createTab({ label, hash }: TabDefinition = {}): string {
  ...
  const labelHtml =
    label === undefined
      ? ''
      : `<div class="wds-tabs__tab-label"><a href="#">${label}</a></div>`;
...
function createTabber(tabs: TabDefinition[], contentBlocks: string[]): string {
  ...
  const contentHtml = contentBlocks
    .map((content) => `<div class="wds-tab__content">${content}</div>`)
    .join('');
...
describe('parseFile coverage', () => {
  it('returns empty results when no tabbers are present', () => {
    expect(parseFile('<div>plain html</div>', 'farming')).toEqual({
      complete: [],
      incomplete: [],
    });
  });
